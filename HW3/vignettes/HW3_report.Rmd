...
```{r analysis-lda-baseline-summary}
...
### Optimal Prior Probability (on Positive Class)

```{r analysis-lda-baseline-optimal}
model$optimal_priors <- calc.optimal.priors()
```




```{r analysis-lda-weighted, include=FALSE}
# Fit model with high cost of misclassification (favoring "Severe")
cost10 <- calc.priors(liver, truth, weight = 10)
model.cost <- fit.model(.data = quote(liver), 
                        algorithm = MASS::lda,
                        formula = severity ~ .,
                        prior = cost10$priors)$obj
print(model.cost)
```
```{r analysis-lda-weighted-table, include=FALSE}
# Make predictions with baseline LDA model.
println("\nWeighted (x10 Cost) misclassification table: ")
model.cost$predict <- predict(model.cost, liver)
model.cost$miss <- (model.cost$predict$class != liver$severity)
(model.cost$table <- table(model.cost$predict$class, liver$severity))
println("\nTotal misclassified:", sum(model.cost$miss), sep = " ")
```

```{r analysis-lda-optimal-error, fig.width=10, fig.size=8, echo=FALSE}
